import { n as log, r as setLogLevel } from "./src-4Uq4GVbL.js";
import { dt as __vitePreload } from "./index-DbiBhjOw.js";
import { n as __name, t as __export } from "./chunk-Y2CYZVJY-DUN_E3Zt.js";
var Channel = {
	min: {
		r: 0,
		g: 0,
...
var detectors = {};
var detectType = /* @__PURE__ */ __name(function(text$1, config2) {
	text$1 = text$1.replace(frontMatterRegex, "").replace(directiveRegex, "").replace(anyCommentRegex, "\n");
	for (const [key, { detector }] of Object.entries(detectors)) if (detector(text$1, config2)) return key;
	throw new UnknownDiagramError(`No diagram type detected matching given configuration for text: ${text$1}`);
}, "detectType");
...
var sanitizeMore = /* @__PURE__ */ __name((text$1, config2) => {
	if (getEffectiveHtmlLabels(config2)) {
		const level = config2.securityLevel;
		if (level === "antiscript" || level === "strict" || level === "sandbox") text$1 = removeScript(text$1);
		else if (level !== "loose") {
			text$1 = breakToPlaceholder(text$1);
			text$1 = text$1.replace(/</g, "&lt;").replace(/>/g, "&gt;");
			text$1 = text$1.replace(/=/g, "&equals;");
			text$1 = placeholderToBreak(text$1);
		}
	}
	return text$1;
}, "sanitizeMore");
var sanitizeText = /* @__PURE__ */ __name((text$1, config2) => {
	if (!text$1) return text$1;
	if (config2.dompurifyConfig) text$1 = purify.sanitize(sanitizeMore(text$1, config2), config2.dompurifyConfig).toString();
	else text$1 = purify.sanitize(sanitizeMore(text$1, config2), { FORBID_TAGS: ["style"] }).toString();
	return text$1;
}, "sanitizeText");
...
var hasBreaks = /* @__PURE__ */ __name((text$1) => {
	return lineBreakRegex.test(text$1);
}, "hasBreaks");
var splitBreaks = /* @__PURE__ */ __name((text$1) => {
	return text$1.split(lineBreakRegex);
}, "splitBreaks");
...
var katexRegex = /\$\$(.*?)\$\$/g;
var hasKatex = /* @__PURE__ */ __name((text$1) => (text$1.match(katexRegex)?.length ?? 0) > 0, "hasKatex");
var calculateMathMLDimensions = /* @__PURE__ */ __name(async (text$1, config2) => {
	const divElem = document.createElement("div");
	divElem.innerHTML = await renderKatexSanitized(text$1, config2);
...
var renderKatexUnsanitized = /* @__PURE__ */ __name(async (text$1, config2) => {
	if (!hasKatex(text$1)) return text$1;
	if (!(isMathMLSupported() || config2.legacyMathML || config2.forceLegacyMathML)) return text$1.replace(katexRegex, "MathML is unsupported in this environment.");
	{
		const { default: katex } = await __vitePreload(async () => {
			const { default: katex$1 } = await import("./katex-CJ94oN9K.js");
		...
		return text$1.split(lineBreakRegex).map((line) => hasKatex(line) ? `<div style="display: flex; align-items: center; justify-content: center; white-space: nowrap;">${line}</div>` : `<div>${line}</div>`).join("").replace(katexRegex, (_, c) => katex.renderToString(c, {
			throwOnError: true,
			displayMode: true,
			output: outputMode
		}).replace(/\n/g, " ").replace(/<annotation.*<\/annotation>/g, ""));
	}
	return text$1.replace(katexRegex, "Katex is not supported in @mermaid-js/tiny. Please use the full mermaid library.");
}, "renderKatexUnsanitized");
var renderKatexSanitized = /* @__PURE__ */ __name(async (text$1, config2) => {
	return sanitizeText(await renderKatexUnsanitized(text$1, config2), config2);
...
var sanitizeText3 = /* @__PURE__ */ __name((text$1) => sanitizeText(text$1, getConfig2()), "sanitizeText");
...
export { purify as $, hasKatex as A, sanitizeText3 as B, getDiagramLoader as C, getThemeVariables3 as D, getSiteConfig as E, renderKatexSanitized as F, setConfig2 as G, setAccDescription as H, reset as I, setupGraphViewbox as J, setDiagramTitle as K, sanitizeCss as L, parseGenericTypes as M, registerDiagram as N, getUrl as O, registerLazyLoadedDiagrams as P, updateSiteConfig as Q, sanitizeDirective as R, getDiagram as S, getEffectiveHtmlLabels as T, setAccTitle as U, saveConfigFromInitialize as V, setConfig as W, styles_default as X, setupGraphViewbox2 as Y, themes_default as Z, frontMatterRegex as _, clear as a, color_default as at, getConfig as b, configureSvgSize as c, defaultConfig2 as d, darken_default as et, defaultConfig_default as f, evaluate as g, directiveRegex as h, calculateMathMLDimensions as i, rgba_default as it, lineBreakRegex as j, getUserDefinedConfig as k, cssStyleSheetToString as l, detectors as m, addDirective as n, adjust_channel_default as nt, commonDb_exports as o, utils_default as ot, detectType as p, setSiteConfig as q, assignWithDepth_default as r, is_dark_default as rt, common_default as s, UnknownDiagramError as t, lighten_default as tt, defaultConfig as u, getAccDescription as v, getDiagramTitle as w, getConfig2 as x, getAccTitle as y, sanitizeText as z };
