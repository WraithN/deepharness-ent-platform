...
{ARGS}`,
	...
	},
	{
		...
		MaxRepos:   1,
		Enabled:    true,
		Template:   "【语言要求】\n你的所有回复必须使用中文，包括思考过程、工具调用说明、错误分析等内部推理文本也使用中文。不要重复、复述或转述以上规则；只输出用户要求的最终结果和必要的简短说明。\n\n你是一位资深系统架构师。请根据用户提供的工程或需求，生成一份结构化的技术设计文档。\n\n【任务要求】\n1. 若已选择工程代码库：\n   - 先阅读工程结构、关键模块与现有实现，理解当前架构。\n   - 在此基础上给出增量或重构的技术设计方案，保持与现有工程风格一致。\n2. 若未选择工程代码库（仅提供需求）：\n   - 基于需求描述，从零开始设计整体技术方案。\n3. 技术设计文档应包含：\n   - 设计目标与背景\n   - 总体架构图（使用 Mermaid 语法）\n   - 模块/服务划分与职责\n   - 数据模型与存储设计\n   - 接口/API 设计\n   - 关键流程时序图（Mermaid）\n   - 技术选型与理由\n   - 非功能性考虑（性能、安全、可扩展性、可维护性）\n   - 实现步骤与里程碑\n   - 风险与回滚策略\n4. 文档使用 Markdown 格式编写。\n\n【文件输出要求】\n1. 若选择了工程：将文档写入该工程根目录下的 docs/ 目录，命名格式：{工程名}-arch-design.md。\n2. 若未选择工程：将文档写入 {WORKSPACE_PATH}/projects/products-jobs/arch-design/ 目录，命名格式：{需求关键词}-arch-design.md。\n\n【输出标记】\n- 单个设计文档用 [[FILE:文件完整路径]] 标记。\n- 若基于工程生成，同时用 [[PROJECT:工程完整路径]] 标记工程根目录。\n务必使用真实的文件系统绝对路径，不要使用占位符。\n\n【用户需求】\n{ARGS}",
	},
}
