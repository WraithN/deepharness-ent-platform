...
func TestMessageBroker_PublishSubscribe(t *testing.T) {
	...
	ev := chat.BrokerEvent{Type: "message", Payload: chat.Message{ID: "msg-1"}}
...
func TestMessageBroker_MultipleSubscribers(t *testing.T) {
	...
	b.Publish(ctx, "sess-1", chat.BrokerEvent{Type: "message", Payload: chat.Message{ID: "msg-1"}})
...
func TestMessageBroker_NoSubscribers(t *testing.T) {
	...
	err := b.Publish(ctx, "sess-1", chat.BrokerEvent{Type: "message"})
	if err != nil {
