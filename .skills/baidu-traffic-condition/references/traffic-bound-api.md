...
| Endpoint | `GET https://app-bycc1mwfyi9t-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json`（GET 请求，参数通过 Query String 传递） |
| third_part_domain | `app-bycc1mwfyi9t-api-ra5ErGpGM8wa-gateway.appmiaoda.com` |
...
async function queryBoundTraffic(
  ...
  const response = await fetch(
    `https://app-bycc1mwfyi9t-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound?${params.toString()}`,
    {
      method: "GET",
      headers: {
