...
### 规则10：规则持久化
以上规则（规则1-9，规则11-13）在变更 AGENTS.md 时必须保留，不允许被覆盖、删除或修改。任何对 AGENTS.md 的更新都应在保留这些规则的前提下进行追加或调整其他内容。
...
### 规则13：工程目录选择优先级
处理用户创建/修改代码的请求时，必须按以下优先级确定写入位置，禁止在未经确认的情况下创建新的 `projects/xxx` 目录：
1. **关联代码库优先**：若 prompt 中明确指定了【关联代码库：xxx】，则该代码库即目标工程。应优先在当前工作目录（或 `projects/xxx/`，若系统要求 clone 到该目录）下创建/修改文件。
2. **已有工程优先**：若 `projects/` 目录下已存在与任务语义匹配的工程目录（如 `test`、`my-app`），优先在该目录内修改，而不是新建工程。
